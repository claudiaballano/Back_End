...
exports.deleteImage = async (req, res) => {
    ...
    // const image = await ImageGallery.findById(idImage)

    // 2. Aplico el método delete después de construir la imagen
    await ImageGallery.deleteOne()
...
exports.updateImage = async (req, res) => {
    ...
    let image = await ImageGallery.findById(idImage)
...
exports.getImages = (req, res, next) => {
    ImageGallery.find().then(allImages => {
...
exports.addImagePost = async (req, res) => {
    ...
    if (id) {
        let splitPredominantColor = predominantColor.split(',')
        await ImageGallery.update({ title, splitPredominantColor, date })
        return res.redirect('/')
    }

    if (await ImageGallery.findOne({ url: imageUrl })) {
